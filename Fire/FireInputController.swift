...
class FireInputController: IMKInputController {
    ...
    private var _originalString = "" {
        didSet {
            ...
            Logger.inputController.notice("[FireInputController] original changed: \(self._originalString, privacy: .public), refresh window")
...
//        }
    ...
    private func hotkeyHandler(event: NSEvent) -> Bool? {
        return Performance.shared.span { _ in
            ...
            if modifiers == [.control, .shift],
               ...
               deleteIndex <= _candidates.count {
                ...
                if target.type != .placeholder {
                    Logger.inputController.notice("hotkey: control + shift + \(deleteIndex, privacy: .public), delete confirm: \(target.text, privacy: .public)")
            ...
            if event.modifierFlags == .control &&
                ...
                Logger.inputController.notice("hotkey: control + \(num, privacy: .public)")
    ...
    private func confirmDelete(_ target: Candidate) {
        Logger.inputController.notice("[FireInputController] confirmDelete: \(target.text, privacy: .public)")
     ...
     func flagChangedHandler(event: NSEvent) -> Bool? {
         ...
         return Performance.shared.span { _ in
             ...
             if !Defaults[.disableEnMode] && Utils.shared.toggleInputModeKeyUpChecker.check(event) {
                 Logger.inputController.notice("[FireInputController]toggle mode: \(self.inputMode.rawValue, privacy: .public)")
             ...
             if event.type == .flagsChanged || (
                ...
                && modifiers != .init(arrayLiteral: .numericPad, .function)
                 ...
                 Logger.inputController.notice("[FireInputController] flagChangedHandler no need handle")
                 return false
             }
             return nil
         }
    ...
    private func predictorHandler(event: NSEvent) -> Bool? {
        return Performance.shared.span { _ in
            ...
            Logger.inputController.notice("[FireInputController] predictorHandler range, selectionRange: \(self.selectionRange(), privacy: .public), replacementRange: \(self.replacementRange(), privacy: .public), client.selectedRange: \(String(describing: self.client()?.selectedRange()), privacy: .public), client.markedRange: \(String(describing: self.client()?.markedRange()), privacy: .public)")
            Logger.inputController.notice("[FireInputController] predictorHandler previous text, \(self._lastInputText, privacy: .public)")
    ...
    private func charKeyHandler(event: NSEvent) -> Bool? {
        return Performance.shared.span { _ in
            ...
            if  _originalString.count <= 0 && match == nil {
                Logger.inputController.notice("非字符,不做处理")
                return nil
            }
            // 当前输入的是英文字符,附加到之前
            if match != nil {
    ...
    func insertText(_ text: String) {
        Logger.inputController.notice("insertText: \(text, privacy: .public)")
        if text.count > 0 {
            var newText = text
            if Defaults[.enableWhitespaceBetweenZhEn] && Utils.shared.shouldConcatWithWhitespace(_lastInputText, text) {
                newText = " " + newText
                Logger.inputController.notice("[FireInputController] insertCandidate should append whitespace: \(newText, privacy: .public)")
            }
